...
public static class TT
{
	...
	public enum enumGameState { Title, Main, Campaign, Play, Paused, Result }
	...
	public const string
		...
		strLOBBY = "LobbyPEEPEE",
		strPUZZLE = "PuzzlePeePee",
		strRESULT = "Result";
	#endregion
	...
	public const float
		latNaju = 35.028333f, //위도
		longNaju = 126.793670f, //경도
	...
	*/
	...
	public static bool zIsNight()
	{
		...
		//Debug.Log("현재 시간 : " + hour);
		return hour >= 18 || hour < 6;
	}

	/*public static bool zIsTest(float latDif, float longDif){
	...
	public static enumGameConcept zGetPlayAnimalMap()
	{
		...
		if (Input.location.isEnabledByUser)
		{
			...
			/*Debug.Log("현재 위도 : " + userLat);
			Debug.Log("현재 경도 : " + userLong);*/
			...
			Debug.Log(string.Format("밤인지? : {0}", zIsNight()));
			Debug.Log(string.Format("나주인지? {0}", zIsCurrInNaju(NajuLatDif, NajuLongDif)));
			Debug.Log(string.Format("절 주변인지? : {0}", zIsNearTemple(TempleLatDif, TempleLongDif)));
			Debug.Log(string.Format("식맛? : {0}", zIsTest(TestLatDif, TestLongDif))); //테스트*/
